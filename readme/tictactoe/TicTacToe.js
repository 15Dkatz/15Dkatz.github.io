...
class TicTacToe extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({grid, endGame: false});
  }
  ...
  *********/
  winner(playerNum, grid) {
    ...
    if (this.arrayWins(diagNeg, playerNum) || this.arrayWins(diagPos, playerNum)) {
      return true;
    }
    return false;
  }
  ...
  opponentMoves(grid, timeout) {
    if (!this.checkGameStatus(grid)) {
      ...
      setTimeout(() => {
        ...
        this.doNothing() :
        this.setState({message: "Your opponent just moved. Your turn!", pending: false});
      }, timeout);
    } else {
      this.checkGameStatus(grid);
    }
  ...
  *********/
  clickableItem(rowNum, itemNum) {
    return this.state.grid[rowNum][itemNum] === 0 && this.state.endGame === false;
  }
